...
- **Order conversion sub-score** counts statuses matching **Delivered**, **Confirmed**, **In Progress**, or **Contract Signed** with **regex word boundaries** (documented in **`momentum_score.json`** as `order_conversion_rule`); the exact rows are listed under **`order_conversion_included`** after each run.
